layui.use(['layer', 'form'], function() {
    ...
    var $ = layui.jquery;
    var apiBase = '/cgi/ThirdParty/ApacheManager/index.cgi';

    function loadStatus() {
        $.getJSON(apiBase + '/api/apache/status', function(res) {
            var statusText = $('#apache-status-text');
            var versionText = $('#apache-version');
    ...
    function apiAction(action, msg) {
        ...
        $.getJSON(apiBase + '/api/apache/' + action, function(res) {
            layer.close(loadIdx);
            if (res.ok) {
    ...
    window.loadLogs = function() {
        $.getJSON(apiBase + '/api/apache/logs', function(res) {
            if (res.ok) {
                var logBox = $('#apache-logs');
    ...
    $('#btn-check-config').click(function() {
        var loadIdx = layer.load(2);
        $.getJSON(apiBase + '/api/apache/check', function(res) {
            layer.close(loadIdx);
            if (res.ok) {
