...
BASE_PATH="$(cd "$SCRIPT_DIR/../www" 2>/dev/null && pwd || echo "/usr/local/apps/@appcenter/fnosnassign/www")"
...
if [ -z "$TRIM_PKGVAR" ]; then
    if [ -d "$INSTALL_ROOT/var" ]; then
        ...
        TRIM_PKGVAR="/var/apps/fnosnassign/var"
    fi
fi

# Auto-adaptive multi-candidate socket locator to support FlyNAS volume dynamic storage
CANDIDATES=(
    "${TRIM_PKGVAR}/fnosnassign.sock"
    "${INSTALL_ROOT//@appcenter/@appdata}/fnosnassign.sock"
    "/vol1/@appdata/fnosnassign/fnosnassign.sock"
    "/var/apps/fnosnassign/var/fnosnassign.sock"
)

BACKEND_UNIX_SOCKET=""
for candidate in "${CANDIDATES[@]}"; do
    if [ -S "$candidate" ]; then
        BACKEND_UNIX_SOCKET="$candidate"
        break
    fi
done

if [ -z "$BACKEND_UNIX_SOCKET" ]; then
    BACKEND_UNIX_SOCKET="${TRIM_PKGVAR}/fnosnassign.sock"
fi
...
proxy_api_request() {
    ...
    local curl_err_log
    curl_err_log="$(mktemp 2>/dev/null || echo "/tmp/curl_err_fnosnassign.log")"
    
    if [ -n "$BACKEND_UNIX_SOCKET" ] && [ -S "$BACKEND_UNIX_SOCKET" ]; then
        curl --unix-socket "$BACKEND_UNIX_SOCKET" "${curl_args[@]}" "$backend_url" 2>"$curl_err_log"
        curl_exit=$?
    else
    ...
    if [ "$curl_exit" -ne 0 ]; then
        local err_msg
        err_msg=$(cat "$curl_err_log" 2>/dev/null || echo "Could not read stderr")
        rm -f "$curl_err_log"
        if [ "$curl_exit" -eq 28 ]; then
            send_text_response "504 Gateway Timeout" "504 Gateway Timeout: Backend request timed out. Details: $err_msg"
        fi
        send_text_response "502 Bad Gateway" "502 Bad Gateway: Backend unavailable (curl exit: $curl_exit, error: $err_msg, socket: $BACKEND_UNIX_SOCKET)"
    fi
    rm -f "$curl_err_log"
