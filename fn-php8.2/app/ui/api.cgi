...
CONF_D_DIR="/etc/php/8.2/fpm/conf.d"
CUSTOM_CONFIG_FILE="$CONF_D_DIR/99-fnos-custom.ini"
FPM_POOL_CONF="/etc/php/8.2/fpm/pool.d/zzz-custom.conf"
...
case "$ROUTE" in
    ...
    "/api/status")
        ...
        if command -v php8.2 >/dev/null 2>&1; then
            php_ver=$(php8.2 -v | head -n1 | grep -o "PHP 8.2.[0-9]*")
        fi
    ...
    "/api/extensions/toggle")
        ...
        if [ -n "$ext_name" ]; then
            if [ -n "$ext_enable" ]; then
                phpenmod -v 8.2 "$ext_name" >/dev/null 2>&1
            else
                phpdismod -v 8.2 "$ext_name" >/dev/null 2>&1
            fi
    ...
    "/api/service/operation")
        # 操作 PHP 8.2 FPM 服务 (start, stop, restart)
        body=$(get_post_body)
