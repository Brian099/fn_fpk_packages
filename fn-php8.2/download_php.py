...
# 结合了我原版为您准备的完整架构清单（如 bz2, dba, gmp, snmp 等），
# 以及补充了您列表里缺少的插件（如 dev, imap 等）。
COMPONENTS_PHP82 = [
...
# 结合了我原本囊括的 (apcu, xdebug, swoole 等) 以及您要求的 (ssh2 等)
COMPONENTS_PECL = {
    "php-redis": "php-redis",
...
def get_latest_deb_url(base_url, package_name):
    print(f"正在从镜像站查询 {package_name} ...")
    req = urllib.request.Request(base_url, headers={'User-Agent': 'Mozilla/5.0'})
    try:
        html = urllib.request.urlopen(req, timeout=10).read().decode('utf-8')
        pattern = fr'href="({package_name}_[^"]+?(?:amd64|all)\.deb)"'
        matches = re.findall(pattern, html)
        
        if not matches:
            print(f"  [失败] 未能找到 {package_name} 的安装包")
            return None
...
def main():
    ...
    for comp in COMPONENTS_PHP82:
        url = get_latest_deb_url(base_php82, comp)
        urls_to_download.append(url)

    # 2. 查询通用组件 php-common
    base_defaults = MIRROR_BASE + "php-defaults/"
    for comp in COMMON_COMPONENTS:
        url = get_latest_deb_url(base_defaults, comp)
        urls_to_download.append(url)

    # 3. 查询独立 PECL 扩展
    for comp, folder in COMPONENTS_PECL.items():
        base_pecl = MIRROR_BASE + folder + "/"
        url = get_latest_deb_url(base_pecl, comp)
        urls_to_download.append(url)

    print(f"\n--- 开始下载离线安装包 (共匹配 {len([u for u in urls_to_download if u])} 个文件) ---")
    total_size = 0
    success_count = 0
