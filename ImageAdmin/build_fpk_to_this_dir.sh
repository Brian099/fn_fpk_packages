...
if [ -f "./serverSourceCode/build_sourceCode.sh" ]; then
    ...
    cp -f "./serverSourceCode/server/amd64/imageadmin_x86_64" "./app/server/amd64/"
    cp -f "./serverSourceCode/server/arm64/imageadmin_aarch64" "./app/server/arm64/"
    echo ">>> 同步完成！"
else
...
# 检查双架构二进制文件
if [ ! -f "./app/server/amd64/imageadmin_x86_64" ]; then
    echo "错误: 未找到 amd64 二进制文件"
    exit 1
fi
if [ ! -f "./app/server/arm64/imageadmin_aarch64" ]; then
