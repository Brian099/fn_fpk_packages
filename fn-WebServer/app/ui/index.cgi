...
if [ "$REL_PATH" = "/api/nginx/status" ] || [ "$REL_PATH" = "/api/apache/status" ] || [ "$REL_PATH" = "/api/web-server/settings" ] || [ "$REL_PATH" = "/api/web-server/type" ] || [ "$REL_PATH" = "/api/sites" ] || [ "$REL_PATH" = "/api/php/status" ] || [ "$REL_PATH" = "/api/fs/list" ] || [ "$REL_PATH" = "/api/sites/create" ] || [ "$REL_PATH" = "/api/settings/get-upload-limit" ] || [ "$REL_PATH" = "/api/settings/set-upload-limit" ] || [ "$REL_PATH" = "/api/sites/update-port" ] || [ "$REL_PATH" = "/api/sites/delete" ] || [ "$REL_PATH" = "/api/sites/enable" ] || [ "$REL_PATH" = "/api/sites/disable" ] || [ "$REL_PATH" = "/api/sites/fix-permissions" ] || [ "$REL_PATH" = "/api/web-server/restart" ] || [ "$REL_PATH" = "/api/db/status" ] || [ "$REL_PATH" = "/api/db/install" ] || [ "$REL_PATH" = "/api/install/log" ]; then
  ...
  if [ "$REL_PATH" = "/api/sites" ]; then
    ...
    ACTION="create-site"
  elif [ "$REL_PATH" = "/api/settings/get-upload-limit" ]; then
    ACTION="get-upload-limit"
  elif [ "$REL_PATH" = "/api/settings/set-upload-limit" ]; then
    ACTION="set-upload-limit"
  elif [ "$REL_PATH" = "/api/sites/update-port" ]; then
    ACTION="update-site-port"
  elif [ "$REL_PATH" = "/api/sites/delete" ]; then
    ACTION="delete-site"
  elif [ "$REL_PATH" = "/api/sites/enable" ]; then
    ACTION="enable-site"
  elif [ "$REL_PATH" = "/api/sites/disable" ]; then
    ACTION="disable-site"
  elif [ "$REL_PATH" = "/api/sites/fix-permissions" ]; then
    ACTION="fix-permissions"
  elif [ "$REL_PATH" = "/api/web-server/restart" ]; then
    ACTION="web-server-restart"
  elif [ "$REL_PATH" = "/api/db/status" ]; then
  ...
  if bash "$BACKEND_SCRIPT" "$ACTION" >"$TMP_OUTPUT" 2>/dev/null; then
    ...
    echo "Content-Type: application/json; charset=utf-8"
    echo ""
    echo '{"error":"backend action failed"}'
  fi
  rm -f "$TMP_OUTPUT"
  exit 0
fi
