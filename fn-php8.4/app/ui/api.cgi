...
FPM_POOL_CONF="/etc/php/8.4/fpm/pool.d/zzz-custom.conf"
...
case "$ROUTE" in
    ...
    "/api/status")
        ...
        if command -v php8.4 >/dev/null 2>&1; then
            php_ver=$(php8.4 -v | head -n1 | grep -o "PHP 8.4.[0-9]*")
        fi
    ...
    "/api/extensions/toggle")
        ...
        if [ -n "$ext_name" ]; then
            if [ -n "$ext_enable" ]; then
                phpenmod -v 8.4 "$ext_name" >/dev/null 2>&1
            else
                phpdismod -v 8.4 "$ext_name" >/dev/null 2>&1
            fi
    ...
    "/api/service/operation")
        # 操作 PHP 8.4 FPM 服务 (start, stop, restart)
        body=$(get_post_body)
