...
var (
	unixSocket = flag.String("unix-socket", "/var/apps/fn-appcenterThirdParty/var/appcenter.sock", "Unix socket path")
...
REM 构建后端服务
echo 构建后端服务...
go build -o output/appcenter.exe ./app/server
if %errorlevel% neq 0 (
    echo 构建后端失败！
    exit /b 1
)
...
# 设置权限
chmod 755 "$TRIM_APPDEST/appcenter"
...
start)
    ...
    # 启动后端服务
    "$TRIM_APPDEST/appcenter" --unix-socket "$TRIM_PKGVAR/appcenter.sock" &
...
stop)
    ...
    # 查找并终止进程
    PID=$(ps aux | grep "appcenter" | grep -v grep | awk '{print $2}')
    if [ -n "$PID" ]; then
        kill $PID
        echo "服务已停止"
    else
